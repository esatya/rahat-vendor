...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('1service-worker.js')
...
$('.form-group .form-control').keyup(function () {
  const inputCheck = $(this).val().length;
  if (inputCheck > 0) {
...
/// ////////////////////////////////////////////////////////////////////////
// Searchbox Toggle
$('.toggle-searchbox').click(() => {
  const a = $('#search').hasClass('show');
  if (a) {
...
$('body').on('click', '.stepper-up', function () {
  const valueInput = $(this).parent('.stepper').children('.form-control');
...
$('.notification-box .close-button').click((event) => {
  event.preventDefault();
  $('.notification-box.show').removeClass('show');
});
// tap to close notification
$('.notification-box.tap-to-close .notification-dialog').click(() => {
  $('.notification-box.show').removeClass('show');
});
/// ////////////////////////////////////////////////////////////////////////
...
// close button toast
$('.toast-box .close-button').click((event) => {
  event.preventDefault();
...
const OnlineText = 'Connected to Internet';
...
$('.custom-file-upload input[type="file"]').each(function () {
  ...
  const $filelabel = $fileUpload.next('label');
  const $filelabelText = $filelabel.find('span');
  const filelabelDefault = $filelabelText.text();
  $fileUpload.on('change', (event) => {
    const name = $fileUpload.val().split('\\').pop();
...
$('.multi-level > a.item').click(function () {
  if ($(this).parent('.multi-level').hasClass('active')) {
    ...
    $(this).parent('.multi-level').parent('ul').children('li')
      .children('ul')
      .slideUp(250);
    $(this).next('ul').slideToggle(250);
    $(this).parent('.multi-level').parent('ul').children('.multi-level')
      .removeClass('active');
...
function AddtoHome(time, once) {
  if (once) {
    const AddHomeStatus = localStorage.getItem('MobilekitAddHomeStatus');
...
/// ////////////////////////////////////////////////////////////////////////
// Dark Mode Detection
const checkDarkModeStatus = localStorage.getItem('MobilekitDarkModeActive');
// if dark mode on
...
$('.dark-mode-switch').change(() => {
  ...
  const darkmodeCheck = localStorage.getItem('MobilekitDarkModeActive');
...
const dmswitch = $('.dark-mode-switch');
