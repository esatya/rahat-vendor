...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  tokenRequest.on("request-success", async (e, d) => {
    (otp.phone = d.phone), (otp.amount = d.tokenAmount);
    otp.open();
  ...
  try {
    ...
    let filteredTx = filterTx(query);
    txTable.loadData(filteredTx.slice(0, 10));
  } catch (error) {}
...
function filterTx(tx) {
  let filteredTx = tx.reduce((acc, current, i) => {
    const x = acc.find((item) => item.phone === current.phone);
    if (!x) {
      if (i < 4) current.date = "2018-08-05";
      else current.date = "2018-08-04";
